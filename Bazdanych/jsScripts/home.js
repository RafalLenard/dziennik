...
jQuery(function ($) { // poczekanie az strona sie w pełni załaduje
    ...
    $('.btnNav').on('click', function () {
        ...
        if (document.getElementById("divActive") == null && $('#' + activId).attr("active") == null) {
            creatNewView(activId);
        }
        else if (document.getElementById("divActive") != null && $('#' + activId).attr("active") == null) { //usuwanie zawartośći i nadpisywanie jej
            hideOutContent();
            setTimeout(function () {
...
function creatNewView(id) {
    if (id == "buttonAddNewGroup") {
        ...
        var schoolDiaryDiary =  $("#divSchoolDiary");
        schoolDiaryDiary.slideToggle(0);
        schoolDiaryDiary.slideToggle(700);

        $('#' + id).attr("active", "true");

    }

    else if(id === "addMarkButton") {
        var divContent = document.getElementById("content");

        var div = createElement(divContent, "DIV", 'markAddition', 'content', '');
        var p = createElement(div, 'H3', '', '', 'Dodaj ocenę');

        writeGroupList("markAddition");

        var button = createElement(div, 'BUTTON', 'buttonGetList', 'approved', 'Wybierz'),
            div2 = createElement(div, "DIV", 'divStudentsList', '', ''),
            flag = createElement(div, 'div', 'divActive', '', '');
...
function writeGroupList(id) {
    $.ajax({
        type: "GET",
        cash: false,
        url: "../project-ai-master/scripts/getGroupList.php",
        dataType: 'json',
        async: false,
        success: function (result) {
...
function writeLessonsList(userData) {
    $.ajax({
        type: "POST",
        cash: false,
        url: "../project-ai-master/scripts/getLessonsList.php",
        dataType: 'json',
        data: {
        ...
        error: function () {
            console.log("Can not find database service");
        }
    });
}
...
function buttonGetStudentsListInit() {
    ...
    $.ajax({
        ...
        url: "../project-ai-master/scripts/getStudentsList.php",
        async: false,
        dataType: 'json',
        success: function (result) {
...
function buttonAddMarkInit() {
    $('.btnAddMark').on('click', function () {
        ...
        $.ajax({
            ...
            url: "../project-ai-master/scripts/addMark.php",
            dataType: 'text',
            success: function (result) {
...
function getShowMoreData(id){
    $.ajax({
        ...
        url: "../project-ai-master/scripts/getAllStudentInfo.php",
        dataType: 'json',
        async: false,
        success: function (result) {
            for (var i = 0; i < result.length; i++) {
...
function removeMark(id){
    $('#' + id + '-removeMark').on('click',function () {
        $.ajax({
            type: "GET",
            cash: false,
            url: "../project-ai-master/scripts/removeMark.php",
            dataType: 'text',
            data: {
                'id': id
            },
...
function addNewGroup(){
    ...
    $.ajax({
        ...
        url: "../project-ai-master/scripts/addGroup.php",
        dataType: 'text',
        success: function (result) {
            console.log(result);
        },
...
function addLesson() {
    $.ajax({
        ...
        url: "../project-ai-master/scripts/addLesson.php",
        dataType: 'text',
        success: function (result) {
            console.log(result);
        },
